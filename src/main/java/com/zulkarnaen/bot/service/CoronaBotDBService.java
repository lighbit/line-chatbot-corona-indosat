...
@Repository
public class CoronaBotDBService {

	@Autowired
	private CoronaBotDao mDao;
	...
	public String findUser(String aUserId) {
		List<CoronaBotUser> self = mDao.getByUserId("%" + aUserId + "%");
	...
	public int joinEvent(String eventID, String userId, String lineID, String displayname) {
		CoronaBotJointEvents joinedEvent = isUserJoinedEvent(eventID, userId);
	...
	private CoronaBotJointEvents isUserJoinedEvent(String eventID, String userID) {
		List<CoronaBotJointEvents> result = mDao.getByJoin(eventID, userID);
	...
	public List<CoronaBotJointEvents> getJoinedEvent(String eventID) {
